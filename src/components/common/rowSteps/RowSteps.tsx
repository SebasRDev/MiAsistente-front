...
const RowSteps = React.forwardRef<HTMLButtonElement, RowStepsProps>(
  (
    ...
    // Cargar datos iniciales del usuario de Clerk
    useEffect(() => {
      if (user) {
        // Obtener datos desde Clerk user y metadata
        const metadata = user.publicMetadata || {};

        setProfileData({
          name: user.firstName || '',
          lastName: user.lastName || '',
          graduate: (metadata.graduate as string) || '',
          profession: (metadata.profession as string) || '',
          id: (metadata.id as string) || '',
          phone: (metadata.phone as string) || '',
          nit: (metadata.nit as string) || '',
          centerName: (metadata.centerName as string) || '',
        });
        ...
        if (!state.user || state.user.uid !== user.id) {
          const userData = {
            ...
            role: (metadata.userType as string) || 'profesional',
            approved: metadata.status === 'active',
            isProfileComplete: (metadata.isProfileComplete as boolean) || false,
    ...
    const updateUserProfile = async () => {
      try {
        ...
        console.log('Making fetch to:', '/api/user/update-profile');
